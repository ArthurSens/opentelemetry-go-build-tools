...
const (
	...
	// go-codeowners expects that repository root is the location for CODEOWNERS file. It will look for ./github/CODEOWNERS by itself.
	// We use a variable just to make this testable.
	pathToCodeOwners = ""
)
...
func (rg *reportGenerator) ingestArtifacts(pathToArtifacts, pathToCodeOwners string) {
	...
	if pathToCodeOwners != "" {
		c, err := cdowners.FromFileWithFS(os.DirFS("./"), pathToCodeOwners)
		if err != nil {
			rg.logger.Warn(
...
func (rg *reportGenerator) processTestResults() {
	for module, suite := range rg.testSuites {
		...
		owners := rg.codeowners.Owners(module)
		report := report{
			module:      module,
...
func (rg *reportGenerator) getRequiredEnv() {
	...
	env[githubOrg] = os.Getenv(githubOrg)
